...
const ServiceCard = ({ margin, type, mainText }) => {
  ...
  return (
    <Card  margin={margin} className="ServiceCard" outlineColor={"neutral"} >
      <Text className="TextInCard" size="16px" weight="600" width={"50%"}>{mainText}</Text>
      <TextCard textColor={color} width={"76px"} height={"30px"} highlightColor={highlightColor} >
        <Text weight="600"  size="12px" >{text}</Text>
      </TextCard>
    </Card>
  )
}
